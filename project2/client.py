...
class Client:
    def __init__(self, n_resources: int) -> None:
        ...
        self.curr_resources: Dict[int, States] = {i: States.RELEASED for i in range(n_resources)}
        self.pub_key = None
...
class ClientUI:
    ...
    def draw_state(self):
        ...
        print()
        print(f"{Fore.YELLOW}Current states: " + {k: v.name for k, v in client.curr_resources.items()})

    def draw_options(self):
        client = self.client

        action = click.prompt("What action you want to take?", type=click.Choice(["ASK", "RELEASE", "NONE"], case_sensitive=False))
        if(action.upper() == "NONE"):
            return
        if(action.upper() == "ASK"):
            if(all(v in (States.HELD, States.WANTED) for v in client.curr_resources.values())):
                click.echo("Holding/Asked all resources already. Unable to ask for more")
                return
        ...
        resource = click.prompt("What resource you want to act on?", type=click.Choice([str(i) for i in range(client.n_resources)]))
