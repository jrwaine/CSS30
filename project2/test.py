...
class TestProject2(unittest.TestCase):
    def test_functional_project(self):
        ...
        clients[0].take_action("ASK", 0)
        time.sleep(0.1)
        self.assertEqual(clients[3].curr_resources, {0: States.WANTED, 1: States.WANTED})
        self.assertEqual(clients[0].curr_resources, {0: States.WANTED, 1: States.WANTED})
        ...
        time.sleep(0.1)
        self.assertEqual(clients[3].curr_resources, {0: States.RELEASED, 1: States.WANTED})
        self.assertEqual(clients[0].curr_resources, {0: States.HELD, 1: States.WANTED})
        clients[0].take_action("RELEASE", 0)

        # Timeout resource 1 in client 2
        time.sleep(6)
        self.assertEqual(clients[3].curr_resources, {0: States.RELEASED, 1: States.HELD})
        self.assertEqual(clients[0].curr_resources, {0: States.RELEASED, 1: States.WANTED})

        # Timeout resource 1 in client 3
        time.sleep(6)
        self.assertEqual(clients[0].curr_resources, {0: States.RELEASED, 1: States.HELD})
